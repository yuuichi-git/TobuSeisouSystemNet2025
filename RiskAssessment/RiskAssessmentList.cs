...
namespace RiskAssessment {
    public partial class RiskAssessmentList : Form {
        ...
        private void CcButtonUpdate_Click(object sender, EventArgs e) {
            this.PutSheetViewList(_riskAssessmentSeminarDao.SelectRiskAssessmentSeminarList(_dateUtility.GetFiscalYearStartDate((int)NumericUpDownExFiscalYear.Value), _dateUtility.GetFiscalYearEndDate((int)NumericUpDownExFiscalYear.Value)));
        }
        ...
        private void SpreadList_CellDoubleClick(object sender, CellClickEventArgs e) {
             ...
             * Detailウインドウを表示
             */
            RiskAssessmentDetail riskAssessmentDetail = new(_connectionVo, _screen, (int)this.NumericUpDownExFiscalYear.Value, ((RiskAssessmentSeminarListVo)SheetViewList.Rows[e.Row].Tag).StaffCode);
            _screenForm.SetPosition(_screen, riskAssessmentDetail);
            riskAssessmentDetail.KeyPreview = true;
            riskAssessmentDetail.Show(this);
            return;
        }
        ...
        /// PutSheetViewList
        /// </summary>
        /// <param name="listRiskAssessmentSeminarListVo"></param>
        private void PutSheetViewList(List<RiskAssessmentSeminarListVo> listRiskAssessmentSeminarListVo) {
            /*
             * SheetViewListの準備
             */
             ...
             */
            int i = 0;
            foreach(RiskAssessmentSeminarListVo riskAssessmentSeminarListVo in listRiskAssessmentSeminarListVo.OrderBy(x => x.Belongs).ThenBy(x => x.UnionCode).ThenBy(x => x.StaffName)) {
                this.SheetViewList.Rows.Add(i, 1);
                this.SheetViewList.RowHeader.Columns[0].Label = (i + 1).ToString();                                                     // Rowヘッダ
                this.SheetViewList.Rows[i].ForeColor = riskAssessmentSeminarListVo.JobForm == 11 ? Color.Blue : Color.Black;                  // 手帳のレコードのForeColorをセット
                this.SheetViewList.Rows[i].Tag = riskAssessmentSeminarListVo;                                                                 // H_RiskAssessmentSeminarListVoを退避
                this.SheetViewList.Cells[i, _colBelongsName].Text = riskAssessmentSeminarListVo.BelongsName;
                this.SheetViewList.Cells[i, _colJobFormName].Text = riskAssessmentSeminarListVo.JobFormName;
                this.SheetViewList.Cells[i, _colOccupation].Text = riskAssessmentSeminarListVo.OccupationName;
                this.SheetViewList.Cells[i, _colUnionCode].Text = riskAssessmentSeminarListVo.UnionCode.ToString();
                this.SheetViewList.Cells[i, _colName].Text = riskAssessmentSeminarListVo.StaffName;
                this.SheetViewList.Cells[i, _colEmploymentDate].Text = riskAssessmentSeminarListVo.EmploymentDate != _defaultDatetime ? riskAssessmentSeminarListVo.EmploymentDate.ToString("yyyy/MM/dd") : string.Empty;
                this.SheetViewList.Cells[i, _colStudentsFlag01].Text = riskAssessmentSeminarListVo.Students01Flag ? "〇" : string.Empty;
                this.SheetViewList.Cells[i, _colStudentsFlag02].Text = riskAssessmentSeminarListVo.Students02Flag ? "〇" : string.Empty;
                this.SheetViewList.Cells[i, _colStudentsFlag03].Text = riskAssessmentSeminarListVo.Students03Flag ? "〇" : string.Empty;
                i++;
            }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void RiskAssessmentList_FormClosing(object sender, FormClosingEventArgs e) {
